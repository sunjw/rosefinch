...
class RosefinchPage {
    ...
    initButtons() {
        ...
        let buttonToNeedSuMode = [
            this.buttonUpload, this.buttonCut, this.buttonCopy, this.buttonPaste, this.buttonDelete,
            this.buttonSetting
        ];
    ...
    onCutCopy(oper) {
        ...
        jqueryUtils.postRestRequest(requestApi, reqObj, function (data) {
            ...
            utils.log('RosefinchPage.onCutCopy, request ERROR, status=%d', respStatus);
            that.hideMainListLoading();
            if (respStatus == 401) {
        ...
        if (this.modalDelete == null) {
            ...
            this.modalDelete.setOkButtonHandler(function () {
                ...
                jqueryUtils.postRestRequest(requestApi, reqObj, function (data) {
                    ...
                    let respStatus = resp.status;
                    utils.log('RosefinchPage.showDeleteDialog, request ERROR, status=%d', respStatus);
                    that.modalDelete.close();
                    if (respStatus == 401) {
                        that.handleRequestSuModeUnauthorized(toastTitle);
                    } else {
